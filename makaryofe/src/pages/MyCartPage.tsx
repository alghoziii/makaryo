...
export default function MyCartPage() {
        ...
        for (const item of cartItems) {
          try {
            ...
            if (service) {
              ...
              console.warn(`Service with slug ${item.slug} is no longer available`);
            }
          } catch (error: unknown) {
            if (error instanceof Error) {
              setError(error.message);
              console.error(`Error fetching service with slug ${item.slug}: ${error.message}`);
              const updateCartAfterError = cartItems.filter((cartItem) => cartItem.slug !== item.slug);
              setCart(updateCartAfterError);
              localStorage.setItem("cart", JSON.stringify(updateCartAfterError));
            }
          }
        }
  ...
  const handleRemoveItem = (slug: string) => {
    ...
    setServiceDetails((prevDetails) => prevDetails.filter((service) => service.slug !== slug));
  };
  ...
  return (
    <main className="relative mx-auto w-full max-w-[640px] overflow-x-hidden bg-white pb-[144px]">
      ...
      <div className="relative flex flex-col gap-[20px] px-5 pt-[100px]">
        <AccordionSection title="Home Services" iconSrc="/assets/images/icons/bottom-booking-form.svg">
                    ...
                    <div className="card flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col gap-1">
                          <h3 className="line-clamp-2 h-[42px] text-sm font-semibold leading-[21px]">{service.name}</h3>
                          <div className="flex items-center gap-[6px]">
                            <div className="flex items-center gap-1">
                              <img src="/assets/images/icons/coint.svg" alt="icon" className="h-4 w-4 shrink-0" />
                              <p className="text-xs leading-[18px] text-shujia-gray">{formatCurrency(service.price)}</p>
                            </div>
                            <div className="flex items-center gap-1">
                              <img src="/assets/images/icons/clock-cart.svg" alt="icon" className="h-4 w-4 shrink-0" />
                              <p className="text-xs leading-[18px] text-shujia-gray">{service.duration} Hours</p>
                            </div>
                          </div>
                        </div>
                      </div>
                      <button onClick={() => handleRemoveItem(service.slug)} type="button" className="shrink-0">
                        <img src="/assets/images/icons/garbage.svg" alt="icon" className="size-[32px] shrink-0" />
                      </button>
                    </div>
                    {index < serviceDetails.length - 1 && <hr className="border-shujia-graylight" />}
                  </div>
                ))
      ...
      <nav className="fixed bottom-5 left-0 right-0 z-30 mx-auto w-full">
        <div className="mx-auto max-w-[640px] px-5">
          <div className="flex items-center gap-[45px] rounded-[24px] bg-shujia-black px-[20px] py-[14px]">
            <div>
              <strong className="whitespace-nowrap text-[22px] font-extrabold leading-[33px] text-white">{formatCurrency(total)}</strong>
              <p className="text-sm leading-[21px] text-white">Grand Total</p>
            </div>
            {cart.length != 0 ? (
              <Link to={`/booking`} className="w-full">
                <p className="w-full rounded-full bg-shujia-orange px-[18px] py-[14px] text-center font-semibold text-white transition-all duration-300 hover:shadow-[0px_4px_10px_0px_#D04B1E80]">Continue</p>
              </Link>
            ) : (
              ""
            )}
